...
def extract_rol_table(pdf_file, output_csv="rol_de_procedimentos.csv"):  #recebe um arquivo PDF e um nome para o arquivo CSV que será gerado
    abbreviations = {"OD": "Ocupação Domiciliar", "AMB": "Ambulatorial"}
    with pdfplumber.open(pdf_file) as pdf:
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                for row in table:
                    cleaned_row = [abbreviations.get(cell.strip(), cell.strip()) if cell else "" for cell in row]  # Substitui abreviações e limpa espaços extras
